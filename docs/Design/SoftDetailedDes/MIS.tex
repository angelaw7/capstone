...
\begin{itemize}
  ...
  \item model: The model will use multiclass logistic regression to predict the
  item categories based on the item names. A vectorizer will be used to convert
  the item names into a format that the model can understand. An encoder will be
  used to convert the predicted category back into a human-readable format. The
  expected accuracy of the model is 90\%.
\end{itemize}
...
\[
\text{Categories} := \{ \text{expense.category} \mid \text{expense} \in \text{Expenses} \}
\]
\[
...
\]\\
\[
\text{remaining}_C := \text{budget}_C - \text{expenses}_C
\]\\
\[
\text{unallocated} := \text{total\_income} - \sum_{C \in \text{Categories}} \text{remaining}_C
\]\\
\[
